...
var _statsModule = _interopRequireDefault(require("three/addons/libs/stats.module.js"));

var _OrbitControls = require("three/addons/controls/OrbitControls");

var _GLTFLoader = require("three/examples/jsm/loaders/GLTFLoader");

var _DRACOLoader = require("three/examples/jsm/loaders/DRACOLoader");

var _FontLoader = require("three/addons/loaders/FontLoader.js");

var _TextGeometry = require("three/addons/geometries/TextGeometry.js");
...
var clipNames = ['fan_rotation', 'fan_rotation.001', 'fan_rotation.002', 'fan_rotation.003', 'fan_rotation.004'];
var aboutCameraPos = {
  x: 0.12,
  y: 0.2,
  z: 0.55
...
var projectsCameraPos = {
  x: 1,
  y: 0.45,
  z: 0.01
};
var projectsCameraRot = {
  x: 0.05,
  y: 0.05,
  z: 0
}; // SCENE & CAMERA
...
camera.position.set(defaultCameraPos.x, defaultCameraPos.y, defaultCameraPos.z); // RENDERER
...
renderer.shadowMap.type = THREE.PCFSoftShadowMap; // STATS
// const stats = new Stats();
// document.querySelector('.experience').appendChild(stats.dom);
// CONTROLS

var controls = new _OrbitControls.OrbitControls(camera, renderer.domElement); // controls.enableDamping = true;
...
controls.update(); // LOAD MODEL & ASSET
...
gltfLoader.load('models/room.glb', function (room) {
  // hide loader on loade
  loaderWrapper.style.display = 'none'; // load video
  ...
  video.autoplay = true;
  video.loop = true; // create video texture
  ...
  room.scene.children.forEach(function (child) {
    ...
    if (child.name === 'Book') {
      bookCover = child.children[0]; // adding texture to book

      var bookTexture = new THREE.TextureLoader().load('textures/book-inner.png');
  ...
  var clips = room.animations;
  clipNames.forEach(function (clipName) {
  ...
  loadIntroText(); // add event listeners
...
var roomLight = new THREE.PointLight(0xffffff, 2.5, 10);
...
roomLight.shadow.camera.far = 2.5; // roomLight.shadow.camera.fov = 100;

roomLight.shadow.bias = -0.002;
scene.add(roomLight); // add light for pc fans

var fanLight1 = new THREE.PointLight(0xff0000, 30, 0.2);
var fanLight2 = new THREE.PointLight(0x00ff00, 30, 0.12);
var fanLight3 = new THREE.PointLight(0x00ff00, 30, 0.2);
var fanLight4 = new THREE.PointLight(0x00ff00, 30, 0.2);
var fanLight5 = new THREE.PointLight(0x00ff00, 30, 0.05);
...
scene.add(fanLight3);
scene.add(fanLight4);
scene.add(fanLight5); // add point light for text on wall

var pointLight1 = new THREE.PointLight(0xff0000, 0, 1.1);
var pointLight2 = new THREE.PointLight(0xff0000, 0, 1.1);
var pointLight3 = new THREE.PointLight(0xff0000, 0, 1.1);
var pointLight4 = new THREE.PointLight(0xff0000, 0, 1.1);
...
scene.add(pointLight4); // SETUP HELPERS
...
function loadIntroText() {
  var loader = new _FontLoader.FontLoader();
  loader.load('fonts/unione.json', function (font) {
    var textMaterials = [new THREE.MeshPhongMaterial({
      color: 0x171f27,
      flatShading: true
    }), new THREE.MeshPhongMaterial({
      color: 0xffffff
    })];
    var titleGeo = new _TextGeometry.TextGeometry('KIOHOCCODE', {
      font: font,
      size: 0.08,
      height: 0.01
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    var subTitleGeo = new _TextGeometry.TextGeometry('Web Designer / Developer / Content Creator / Game Dev', {
      font: font,
      size: 0.018,
      height: 0
    });
...
function switchTheme(themeType) {
  if (themeType === 'dark') {
    ...
    document.body.classList.add('dark-theme'); // main lights

    _gsap["default"].to(roomLight.color, {
      r: 0.27254901960784313,
      g: 0.23137254901960785,
      b: 0.6862745098039216
    });

    _gsap["default"].to(ambientLight.color, {
      r: 0.17254901960784313,
      g: 0.23137254901960785,
      b: 0.6862745098039216
    });

    _gsap["default"].to(roomLight, {
      intensity: 1.5
    });

    _gsap["default"].to(ambientLight, {
      intensity: 0.3
    }); // fan lights


    _gsap["default"].to(fanLight5, {
      distance: 0.07
    }); // text color


    _gsap["default"].to(titleText.material[0].color, {
      r: 8,
      g: 8,
      b: 8,
      duration: 0
    ...
    _gsap["default"].to(titleText.material[1].color, {
      r: 5,
      g: 5,
      b: 5,
      duration: 0
    ...
    _gsap["default"].to(subtitleText.material[0].color, {
      r: 8,
      g: 8,
      b: 8,
      duration: 0
    ...
    _gsap["default"].to(subtitleText.material[1].color, {
      r: 5,
      g: 5,
      b: 5,
      duration: 0
    ...
    _gsap["default"].to(pointLight1, {
      intensity: 0.6
    });

    _gsap["default"].to(pointLight2, {
      intensity: 0.6
    });

    _gsap["default"].to(pointLight3, {
      intensity: 0.6
    });

    _gsap["default"].to(pointLight4, {
      intensity: 0.6
    });
  } else {
    lightSwitch.rotation.z = 0;
    document.body.classList.remove('dark-theme');
    document.body.classList.add('light-theme'); // main light
...
function resetBookCover() {
  ...
  _gsap["default"].to(bookCover.rotation, {
    x: 0,
    duration: 1.5
  });
}
...
function resetProjects() {
  ...
  projects.forEach(function (project, i) {
    _gsap["default"].to(project.mesh.material, {
      opacity: 0,
      duration: 1
    });

    _gsap["default"].to(project.mesh.position, {
      y: project.y,
      duration: 1
    });

    _gsap["default"].to(project.mesh.scale, {
      x: 0,
      y: 0,
      z: 0,
      duration: 0,
...
function resetCamera() {
  ...
  _gsap["default"].to(camera.position, _objectSpread({}, defaultCameraPos, {
    duration: 1.5
  }));

  _gsap["default"].to(camera.rotation, _objectSpread({}, defaultCamerRot, {
    duration: 1.5
  }));

  _gsap["default"].delayedCall(1.5, enableOrbitControls); // reset dimmed light for about display
...
function cameraToAbout() {
  ...
  _gsap["default"].to(camera.position, _objectSpread({}, aboutCameraPos, {
    duration: 1.5
  }));

  _gsap["default"].to(camera.rotation, _objectSpread({}, aboutCameraRot, {
    duration: 1.5
  }));
...
function aboutMenuListener() {
  document.getElementById('about-menu').addEventListener('click', function (e) {
    ...
    _gsap["default"].delayedCall(1.5, enableCloseBtn);
  });
}
...
function projectsMenuListener() {
  ...
  projects.forEach(function (project, i) {
    var colIndex = i % 3 === 0 ? 0 : 1;
    var rowIndex = Math.floor(i / 3);
    var geometry = new THREE.PlaneGeometry(0.71, 0.4);
    ...
    var projectPlane = new THREE.Mesh(geometry, material);
    projectPlane.name = 'project';
    projectPlane.userData = {
      url: project.url
    };
    projectPlane.position.set(0.3 + i * 0.8 * colIndex, 1 - rowIndex * 0.5, -1.15);
    projectPlane.scale.set(0, 0, 0); // mesh & y vars needed for animation
  ...
  document.getElementById('projects-menu').addEventListener('click', function (e) {
    ...
    _gsap["default"].to(camera.position, _objectSpread({}, projectsCameraPos, {
      duration: 1.5
    }));

    _gsap["default"].to(camera.rotation, _objectSpread({}, projectsCameraRot, {
      duration: 1.5
    }));

    _gsap["default"].delayedCall(1.5, enableCloseBtn); // animate & show project items
...
function init3DWorldClickListeners() {
  ...
  window.addEventListener('click', function (e) {
    ...
    var newTheme = theme === 'light' ? 'dark' : 'light'; // prevent about focus on button click which are positioned above book in mobile view

    var closeBtn = document.getElementById('close-btn');
    var projectsBtn = document.getElementById('projects-menu');
...
document.addEventListener('mouseup', function (e) {
  var container = document.querySelector('.contact-menu');
